...
import { ApiResponse, LazyParams, PriceEntry, ProductPriceRecordParsed } from '@/types/marginData';



export default function MarginData() {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const response = await axios.get<ApiResponse>('/ajaxProductsMargin', {
                params: {
            ...
            const parsedData = response.data.data.map((item) => ({
                ...item,
                price_data: JSON.parse(item.price_data),
            }));
        ...
        <ColumnGroup>
            ...
            <HeaderRow>
                {dates.flatMap((date) => [
                    ...
                    <Column key={`${date}-dp`} header="Discount Price" />,
